...
turtles-own [
  ...
  cooperation-count      ;;number of times the partner cooperated
  total-interactions     ;;total number of interactions with the partner
]
...
to prepare-next-round
  ...
  ;; Update the cooperation count and total interactions
  ask turtles [
    if not defect-now? [
...
to custom-strategy ;;Turtle Procedure
  ...
  [
    let cooperation-frequency cooperation-count / total-interactions
    ;; If the opponent cooperates more than 50% of the time, assume they are cooperative and reciprocate
    ifelse cooperation-frequency > 0.5 [
      ;; If the opponent has defected in the last two rounds, assume they are defecting and defect
      ifelse partner-defected? and partner-defected-past? [
        set defect-now? true
      ]
      ;; Otherwise, play tit for tat
      [
        set defect-now? partner-defected?
      ]
    ]
    ;; If the opponent cooperates less than or equal to 50% of the time, always defect
    [
      set defect-now? true
    ]
  ]
