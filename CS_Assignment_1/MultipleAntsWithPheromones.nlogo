...
turtles-own [
  carrying-food?
  ;; max-turn-angle       ;; maximum angle by which the ant can turn
  ;; max-step-size        ;; maximum distance the ant can move in one step
]
...
to setup-turtles
  create-turtles population [
    ...
    set max-turn-angle max-turn-angle  ;; set max turn angle from interface slider
    set max-step-size max-step-size    ;; set max step size from interface slider
  ]
end
...
to wander  ;; turtle procedure
  ...
  if not can-move? max-step-size [ rt 180 ]
  fd max-step-size
end
...
@#$#@#$#@
<experiments>
  <experiment name="CS_Assignment_1.1" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
