...
to setup-experiment [ diffusion evaporation ]
  set diffusion-rate diffusion
  set evaporation-rate evaporation
  setup
end

to run-experiment
  while [all? patches [food = 0] and all? turtles [not carrying-food?]] [
    go
  ]
  set experiment-result ticks
end
...
to setup
  ...
  setup-patches
  setup-turtles
  set diffusion-rate 1 + random 10
  set evaporation-rate 10 + random 40
  reset-ticks
  set experiment-result 0
end
...
to go  ;; forever button
  if all? patches [food = 0] and all? turtles [not carrying-food?] [
    set experiment-result ticks
    stop
  ]
  ask turtles [
...
"" ""
PENS
"Pen" 2.0 2 -2805978 true "if not empty? list-ticks [plotxy last list-population last list-ticks]" ""
...
<experiments>
  <experiment name="experiment" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>experiment-result</metric>
