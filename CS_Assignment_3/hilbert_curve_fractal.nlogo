globals [
  ...
  D       ; down
  R       ; right
  step    ; width of each step of the trail
  agent
  waitsecs
  line-color
  degree
  current-step ; current step in the Hilbert curve
  total-steps  ; total number of steps in the Hilbert curve
  N            ; number of smaller copies
  M            ; scale factor
  fractal-length ; length of the fractal
]
...
to setup
  ...
  set R  90
  set degree 4
  crt 1 [setxy (max-pxcor - 1) (min-pycor + 1) set heading D set agent self pen-down]
  set waitsecs exp (- degree)
  set current-step 0
  set total-steps (4 ^ degree) ; each level of the Hilbert curve has 4 times more steps than the previous level
  set N 4 ; for a Hilbert curve, there are 4 smaller copies at each iteration
  set M 0.5 ; for a Hilbert curve, each line segment shrinks by a factor of 0.5 at each iteration
  set fractal-length 0 ; initialize the length of the fractal
end

to go
  if current-step < total-steps [
    ask turtles [
      hilbert degree ([heading] of agent)
      set fractal-length fractal-length + step ; update the length of the fractal
    ]
    tick
  ]
  ;output-print (word "N: " N ", M: " M ", Hausdorff Dimension: " (log N / log (1 / M)) ", Fractal Length: " fractal-length)
end
...
to move [dir]
  if current-step < total-steps [
    ...
    display wait waitsecs
    set current-step current-step + 1
  ]
end

to hilbert [level dir]
  ifelse level <= 1 [
    if dir = L [move R move D move L]
    if dir = R [move L move U move R]
    if dir = U [move D move R move U]
    if dir = D [move U move L move D]
  ]
  [
    if dir = L [
      hilbert (level - 1) U
