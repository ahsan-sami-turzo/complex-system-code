turtles-own [energy-level age reproduction-rate]
globals [last-count hatched-count died-count total-hatched total-died]
...
to setup
  ...
  set total-hatched 0
  set total-died 0
  create-turtles initial-population [initialize-turtles]
end

to initialize-turtles
  set energy-level energy-level-slider
  set reproduction-rate reproduction-rate-slider
  set age 0
  set color white
  set shape "bunny2"
  set size 2
...
to reproduce
  let current-hatch count turtles
  if random-float 1.0 < birthrate and count turtles < carrying-capacity [
    hatch floor reproduction-rate [inherit-traits]
  ]
  set hatched-count hatched-count + (count turtles - current-hatch)
  set total-hatched total-hatched + hatched-count
end

;to expire
;  let current-count count turtles
;  if energy-level <= 0 or age > max-age-slider [die]
;  set died-count died-count + (current-count - count turtles)
...
to setup-plot
  ...
  create-temporary-plot-pen "Population"
  set-plot-pen-color black

  create-temporary-plot-pen "Hatched"
  set-plot-pen-color green

  create-temporary-plot-pen "Died"
...
to update-plot
  ...
  set-current-plot-pen "Hatched"
  plot hatched-count

  set-current-plot-pen "Died"
  plot died-count
end
