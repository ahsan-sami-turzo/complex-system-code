...
globals [time-to-synchronize not-flashing]

to setup
  clear-all
  set time-to-synchronize 0
  set not-flashing 0
...
to go
  ...
  if all? turtles [color = yellow] or ticks > 9000 [
    set time-to-synchronize ticks
    set not-flashing count turtles with [color != yellow]
    stop
  ]
...
@#$#@#$#@
<experiments>
  <experiment name="fireflies" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>time-to-synchronize</metric>
    <steppedValueSet variable="number" first="500" step="500" last="2000"/>
    <steppedValueSet variable="cycle-length" first="5" step="5" last="20"/>
    <steppedValueSet variable="flash-length" first="1" step="2" last="9"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
