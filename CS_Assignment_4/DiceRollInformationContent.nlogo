...
globals [
  txt freq-table probabilty-table word-count Max-Word-Count result die-counts
]

to setup
  ca
  set roll-results ""
  set die-counts [0 0 0 0 0 0]
  set Max-Word-Count 1000
  crt 1 [set shape "die 1" set size 14 set color gray]
end
...
to-report list-of-words
  ...
  [ let n position " " $txt
    ...
    if not empty? $item [set words lput $item words]  ; append to list
...
to roll-die
  set result random 6 + 1 ; Generate a random number between 1 and 6
  show-roll
end
...
to show-roll
  ...
  let die-result result
  set die-counts replace-item (die-result - 1) die-counts (item (die-result - 1) die-counts + 1)
  set txt (word txt " " die-result)
end
